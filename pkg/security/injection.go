...
// Implements SPEC-SEC-02: Prompt Injection
...
func (d *PromptInjectionDetector) initPatterns() {
	patterns := []struct {
		...
		{`(?i)ignore\s+(all\s+)?(previous|above|the)\s+(instructions?|prompts?|commands?)`, SeverityCritical, "override"},
		...
		{`(?i)you\s+are\s+now\s+(a\s+)?new\s+(AI|assistant|persona)`, SeverityCritical, "role_confusion"},
		{`(?i)from\s+now\s+on\s+you\s+are`, SeverityCritical, "role_confusion"},
		{`(?i)act\s+as\s+(if\s+you\s+are\s+(a\s+))?(different|another)`, SeverityHigh, "role_confusion"},
		...
		{`(?i)(jailbreak|jail\s*break)\s*(mode|technique|method)`, SeverityHigh, "jailbreak"},
		...
		{`(?i)whatever\s+happens,\s+(do|say|output)\s+`, SeverityMedium, "manipulation"},
		{`(?i)no\s+matter\s+what,\s+(do|say|output)\s+`, SeverityMedium, "manipulation"},
